...
TEST_CASE("SwitchBot Outdoor Meter metadata response parses") {
    const auto metadata = parseMetadataResponse(hexToBytes("0169b59d5269fc354600012d34003c"));
...
TEST_CASE("SwitchBot Outdoor Meter metadata response rejects malformed packets") {
    CHECK_FALSE(parseMetadataResponse({}).has_value());
    CHECK_FALSE(parseMetadataResponse(hexToBytes("0269b59d5269fc354600012d34003c")).has_value());
    CHECK_FALSE(parseMetadataResponse(hexToBytes("0169b59d5269fc354600012d340000")).has_value());
}

TEST_CASE("SwitchBot Outdoor Meter index epoch conversion") {
    const uint32_t startEpoch = 1773509975U;
    CHECK(epochForIndex(startEpoch, 77061, 60) == 1778133635U);
    CHECK(indexForEpochFloor(startEpoch, 1778133635U, 60) == 77061U);
    CHECK(indexForEpochCeil(startEpoch, 1778133636U, 60) == 77062U);
    CHECK(pageStartForIndex(77061U) == 77058U);
}

TEST_CASE("SwitchBot Outdoor Meter page decoder decodes six samples") {
    const uint32_t startEpoch = 1773509975U;
    const uint16_t interval = 60;
    const auto samples = decodePageResponse(hexToBytes("018b32668b328b32668b328b32668b32"),
                                            77061,
                                            startEpoch,
                                            interval);
...
TEST_CASE("SwitchBot Outdoor Meter page decoder handles mixed decimal nibbles") {
    const uint32_t startEpoch = 1773509975U;
    const auto samples = decodePageResponse(hexToBytes("018b32688b328b32828c32"), 77067, startEpoch, 60);
    REQUIRE(samples.has_value());
    REQUIRE(samples->size() == 6);
    CHECK((*samples)[0].temperatureC == doctest::Approx(11.6));
...
TEST_CASE("SwitchBot Outdoor Meter page decoder rejects malformed packets") {
    CHECK_FALSE(decodePageResponse({}, 0, 0, 60).has_value());
    CHECK_FALSE(decodePageResponse(hexToBytes("02"), 0, 0, 60).has_value());
    CHECK_FALSE(decodePageResponse(hexToBytes("018b32668b328b32668b328b32668b32"), 0, 0, 0).has_value());
}

TEST_CASE("SwitchBot Outdoor Meter later page matches known decoded output") {
    const uint32_t startEpoch = 1773509975U;
    const auto samples = decodePageResponse(hexToBytes("018c31558c318c31578c2f8c2f778c2f"), 77079, startEpoch, 60);
    REQUIRE(samples.has_value());
    REQUIRE(samples->size() == 6);
